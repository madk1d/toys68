...
<link rel="stylesheet" href="/assets/css/main.css<?php echo "?$cssver";?>" />
	...
	<body class="is-preload">
			...
			<div id="main">
						...
						<span style="text-align:center;float:right;margin-top:1.8em;margin-right:10px;"><b>Архив</b> <?php echo ( isset($_GET['email_sort']) ? " ".$_GET['email_sort'] : ""); ?></span> 
						 ...
						 <div style="margin-top:2em;"></div>
							...
							if (!isset($_GET['email_sort'])) {
								...
								if (isset($_GET['sort'])&&$_GET['sort']=="1")
										echo "<button class='button3' style='margin:5px;' onclick=\"location.href='/cabinet/archive/'\"><img style='width:33px;height:33px;' src='../stop.jpg' title='Отменить сортировку'></button>"; 
								else
						 ...
						 ?>
													...
													if (!isset($_GET['email_sort'])&&isset($_GET['sort'])) {
														...
														$result  =  mysqli_query( $base,  "SELECT DISTINCT email FROM archive" );
	...
	if (isset($_GET['del'])) 
{
	...
	$query="DELETE FROM archive WHERE id = '".$_GET['del']."'";
mysqli_query($base, $query);
}
	
	if (isset($_GET['id'])) {
$query = "UPDATE archive SET status='Выполнено' WHERE id='".$_GET['id']."'";
mysqli_query($base, $query);
	}
	...
	$result  =  mysqli_query( $base,  "SELECT * FROM archive WHERE email='".$_GET['email_sort']."'");	
	...
	if ( !$result ) {echo "Произошла ошибка: "  .  mysqli_error(); exit();}
				...
				$i=1;
						while ( $row = mysqli_fetch_assoc($result) )
						{
							...
							echo "<td style='vertical-align : middle;'><a class='button small' href='?email_sort=".$row['email']."&id=".$row['id']."#z".$i."'><span style='color:rgb(0,200,0);'>Готов</span></a>"."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class='button small' href='?del=".$row['id']."#z".($i>6?$i-1:0)."'><span style='color:rgb(200,0,0);'>Del</span></a>"."</td>";
							else {
								echo "<td style='vertical-align : middle;'><button class='button3' style='margin:5px;' onclick=\"location.href='?email_sort=".$row['email']."&del=".$row['id']."#z".($i>6?$i-1:0)."'\"><img style='width:33px;height:33px;' src='../del.jpg' title='Удалить заказ'></button></td>";
	...
	$result  =  mysqli_query( $base,  "SELECT * FROM archive" );
			...
			if ( !$result ) {echo "Произошла ошибка: "  .  mysqli_error(); exit();}
				...
				$i=1;
						while ( $row = mysqli_fetch_assoc($result) )
						{
							...
							if ($row['status']=="В процессе")
							echo "<td style='vertical-align : middle;'><a class='button small' href='?id=".$row['id']."#z".$i."'><span style='color:rgb(0,200,0);'>Готов</span></a>"."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class='button small' href='?del=".$row['id']."#z".($i>6?$i-1:0)."'><span style='color:rgb(200,0,0);'>Del</span></a>"."</td>";
							else {
								echo "<td style='vertical-align : middle;'><button class='button3' style='margin:5px;' onclick=\"location.href='?del=".$row['id']."#z".($i>6?$i-1:0)."'\"><img style='width:33px;height:33px;' src='../del.jpg' title='Удалить заказ'></button></td>";
								$sum=$sum+$row['num']*$row['item_p'];}
							echo "</tr>";
						$i++;
						}
